...
var TrialTool = (function(){
		...
		var isReady = function(funcName, args){
			if (!consoleWindow.ConsoleHelper) {
				...
				window.setTimeout(function(){
					if (typeof result[funcName] === "function") result[funcName].apply(consoleProxy, args);
				}, 1000);
			}
	...
	var getDependencies = function(example){
		...
		for (var i = 0; i < visitedNodes.length; i++) {
			if (node.get(0) === visitedNodes[i]) return [];
		}
	...
	var showCodeWithDependencies = function(){
		...
		$.each(getDependencies(currentSelection), function(){
			if (this.insertAt) {
				codeToinsert[this.insertAt] = this.code;
			} else {
		...
		for (var insertPoint in codeToinsert) {
			var regEx = new RegExp("/\\*[\\s]*Code for \\${" + insertPoint + "}[\\s]*\\*\\/", "ig");
			finalCode = finalCode.replace(regEx, codeToinsert[insertPoint]);
		}
